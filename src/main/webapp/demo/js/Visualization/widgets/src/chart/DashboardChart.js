(function (root, factory) {
    ...
    DashboardChart.prototype.setChartData = function (actualData,chartType) {        
		...
		var subject = ["subject"];
		var column = myData[0]['data']['rows'][0];
		...
		{
			var pieChartData=[];			
			for(i = 0; i<myData[0]['data']['rows'][0].length; i++)	{	
				var newData=[];
				newData[0] = myData[0]['data']['rows'][0][i];
				newData[1] = myData[0]['data']['rows'][1][i];						
		...
		{	
			...
			for (i=0;i<myData[0]['axis']['x']['categories'].length;i++)
			{				
				var columnSubject = [];
				columnSubject[0] = myData[0]['axis']['x']['categories'][i];				
				var columnInfo = myData[0]['data']['rows'][i+1];				
